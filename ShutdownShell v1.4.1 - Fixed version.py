...
def edit_profile(profile_name: str) -> bool:
    ...
    if ptype == 'network':
        ...
        if s.get('monitor_disk', False):
            print(f"Текущий мониторинг дисков: ВКЛ ({s.get('monitor_disk_type', 'all')})")
            if s.get('monitor_disk_type') == 'specific':
                print(f"Мониторинг диска: {s.get('monitor_disk_letter', 'Не указан')}")
        else:
            print("Текущий мониторинг дисков: ВЫКЛ")
        
        monitor_disk_choice = input("Изменить мониторинг дисков? (y/n) [n]: ").lower() or 'n'
            ...
            if new_monitor == 'y':
                ...
                if disk_mon_type == '2':
                    ...
                    if disk_letter is False:
                        ...
                        new['monitor_disk'] = s.get('monitor_disk', False)
        ...
        print(f"Текущий диск: {s.get('disk_letter', 'Все диски')}")
