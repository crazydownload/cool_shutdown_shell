    ...
    try:
        while True:
            try:
                ...
                choice = input("\nВыберите вариант (1/2/3/4/5/6/7/8): ")
                ...
                if choice == "1":
                    ...
                    try:
                        ...
                        if 'disk_threshold' in selected_profile:
                            # Это профиль мониторинга дисков
                            while True:
                    ...
                    print("\n=== Режим мониторинга активности дисков ===")
                    ...
                    try:
                        allowed_failures = int(input(f"Допустимые пропуски [по умолчанию: {allowed_failures}]: ") or allowed_failures)
                        threshold = float(input(f"Пороговая скорость (МБ/с) [по умолчанию: {threshold}]: ") or threshold)
                        interval = int(input(f"Интервал проверки (сек) [по умолчанию: {interval}]: ") or interval)
                    ...
                    while True:
                        should_restart = monitor_disk_only(allowed_failures, threshold, interval, shutdown_delay, action_mode)
                        if not should_restart:
                            return
                        print("\nНажмите Enter для возврата в меню или любую другую клавишу для перезапуска мониторинга...")
                        if msvcrt.getch() == b'\r':
