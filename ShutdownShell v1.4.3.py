...
def check_disk_activity_single(disk_letter: str, threshold_mb=1.0, sample_interval=0.5) -> bool:
    ...
    try:
        ...
        if not disk_io:
            print("❌ Нет данных о дисковых операциях")
            return False
...
def edit_profile(profile_name: str) -> bool:
    ...
    if ptype == 'network':
        ...
        ttype = input(f"\nТип трафика (u - upload / d - download) [{old_traffic}]: ").lower()
        if ttype in ('u', 'd'):
            new['traffic_type'] = ttype
        else:
            new['traffic_type'] = old_traffic
        show_change("Тип трафика", 
                   '📤 Upload' if old_traffic == 'u' else '📥 Download',
                   '📤 Upload' if new['traffic_type'] == 'u' else '📥 Download')
        ...
        monitor_disk_choice = input("\nИзменить мониторинг дисков? (y/n) [n]: ").lower() or 'n'
        if monitor_disk_choice == 'y':
            new_monitor = input("Включить мониторинг дисков? (y/n) [n]: ").lower() or 'n'
            if new_monitor == 'y':
                new['monitor_disk'] = True
                print("\n1. Мониторинг всех дисков (общая активность)")
                print("2. Мониторинг конкретного диска")
                disk_mon_type = input("Выберите тип мониторинга (1/2) [1]: ") or '1'
        ...
        if input("Сменить диск? (y/n): ").lower() == 'y':
        ...
        delay = input(f"\nЗадержка (текущая: {format_time(old_delay)}): ")
        if delay:
        ...
        am = input(f"\nРежим действия (s/r/h/b) [{old_action_mode}]: ").lower()
        if am in ('s', 'r', 'h', 'b'):
            new['action_mode'] = am
        else:
            new['action_mode'] = old_action_mode
        show_change("Действие", action_names.get(old_action_mode, old_action_mode), 
                   action_names.get(new['action_mode'], new['action_mode']))
...
def create_new_profile():
    ...
    if tp == '1':  # Мониторинг сети
        ...
        print(f"\n{'─' * 40}")
        print("📊 Тип трафика:")
        print("u - 📤 Upload (исходящий трафик)")
        print("d - 📥 Download (входящий трафик)")
        tt = input("\nВыберите тип трафика (u/d) [d]: ").lower() or 'd'
        if tt not in ('u', 'd'):
            tt = 'd'
        settings['traffic_type'] = tt
        ...
        delay = input("\n⏳ Задержка до действия (пример: 1h30m) [30s]: ")
        sd = parse_time_input(delay) if delay else 30
        settings['shutdown_delay'] = sd
        
        # Режим действия
        print(f"\n{'─' * 40}")
        print("⚡ Режим действия:")
        print("s - Выключение")
        print("r - Перезагрузка")
        print("h - Спящий режим")
        print("b - Звуковой сигнал")
        mode = input("\nВыберите режим (s/r/h/b) [s]: ").lower() or 's'
        if mode not in ('s', 'r', 'h', 'b'):
            mode = 's'
        ...
        monitor_disk_choice = input("\n📊 Включить мониторинг дисков? (y/n) [n]: ").lower() or 'n'
        ...
        # Задержка
        delay = input("\n⏳ Введите время задержки (пример: 1h30m15s): ")
        if not delay:
    ...
    if tp == '1':
        ...
        print(f"  📊 Тип трафика: {'📤 Upload' if settings.get('traffic_type') == 'u' else '📥 Download'}")
        ...
        print(f"  ⚡ Действие: ", end="")
        action_names = {'s': 'Выключение', 'r': 'Перезагрузка', 'h': 'Спящий режим', 'b': 'Звук'}
        print(action_names.get(settings.get('action_mode', 's'), 'Выключение'))
        ...
        while True:
            ...
            if ch == '1':
                ...
                try:
                    ...
                    delay = input("\n⏳ Задержка до действия [30s]: ")
                    sd = parse_time_input(delay) if delay else 30
                    mode = input("\n⚡ Режим (s/r/h/b) [s]: ").lower() or 's'
                    
                    while True:
                        should_restart = monitor_disk_only(af, thr, interval, sd, mode, disk_letter)
                        if not should_restart:
                            return
                        print("\nНажмите Enter для возврата в меню или любую другую клавишу для перезапуска мониторинга...")
                        if msvcrt and msvcrt.getch() == b'\r':
